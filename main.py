...
class EnemyShip(SpaceShip):
    ...
    def update(self):
        ...
        if collided:
            ...
            collided.kill()
            game.score += 1
            game.hud.score.update(f'Score: {game.score}')
        if self.shoot_cd.done():
            game.enemies_lasers.add(
                Laser(self.rect.centerx, self.rect.bottom, RED, 5)
            )
            self.shoot_cd.reset()
...
class HUD():
    def __init__(self):
        ...
        self.restart = Label('Press ENTER to restart', WIDTH / 2, HEIGHT / 2 + 70, 40)
